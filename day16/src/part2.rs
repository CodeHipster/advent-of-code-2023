...
pub(crate) fn part2(file: &String) {
  ...
  for c in 0..contraption.cols(){
    start_states.push(State{row: 0, col: c, direction: Direction::DOWN});
    start_states.push(State{row: contraption.rows()-1, col: c, direction: Direction::UP});
  }
  for r in 0..contraption.rows(){
    start_states.push(State{row: r, col: 0, direction: Direction::RIGHT});
    start_states.push(State{row: r, col: contraption.cols()-1, direction: Direction::LEFT});
  }

  let answer = start_states.into_iter().map(|state|energized(state, &contraption)).max().unwrap();
...
fn energized(start_state: State, contraption: &Grid<char>)->usize{
  ...
  visited.iter().fold(HashSet::new(), |mut acc, s|{
